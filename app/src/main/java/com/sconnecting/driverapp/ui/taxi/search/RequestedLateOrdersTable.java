...
public class RequestedLateOrdersTable extends RecyclerView.Adapter<RequestedLateOrdersCell> {

    List<DriverBidding> dataSource;
    ...
    public RequestedLateOrdersTable(RecyclerView table, SwipeRefreshLayout refreshCtrl){
    ...
    public void reloadData(final Completion listener ) {
        ...
        List<DriverBidding> data = new ArrayList<>();

        reloadData(data,listener);

    }

    public void reloadData( List<DriverBidding> data,final Completion listener) {
    ...
    public void appendData( List<DriverBidding> data,final Completion listener) {
    ...
    @Override
    public RequestedLateOrdersCell onCreateViewHolder(ViewGroup viewGroup, int viewtype) {


        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.taxi_late_requestedlateorder_cell,viewGroup,false);
        return new RequestedLateOrdersCell(view);


    }

    @Override
    public void onBindViewHolder(RequestedLateOrdersCell cell, int i) {

        DriverBidding item = dataSource.get(i);
        cell.updateWithModel(item);
        cell.bind(item, new RequestedLateOrdersCell.OnItemClickListener() {
            @Override
            public void onItemClick(final DriverBidding bidding) {


                new TravelOrderController().getById(true, bidding.TravelOrder, new GetOneListener() {
                    @Override
                    public void onGetOne(Boolean success, BaseModel item) {
                        ...
                        if(order != null){
                            ...
                            if(order.IsMonitoring()) {

                                if(((RequestedLateOrdersScreen)tableView.getContext()).Caller.equals("OrderScreen") &&  SCONNECTING.orderScreen != null){

                                    ((RequestedLateOrdersScreen)tableView.getContext()).onBackPressed();
                                    ((RequestedLateOrdersScreen)tableView.getContext()).overridePendingTransition(R.anim.pull_in_right, R.anim.push_out_left);
    ...
    public void loadData(final Completion listener){
        ...
        TravelOrderController.GetBiddingsByDriver(SCONNECTING.driverManager.CurrentDriver.getId(), new GetItemsListener() {
            @Override
